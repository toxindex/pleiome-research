...
# pyspark is only needed for the spark-based fit()/transform() (tokenizer-build path),
# not for loading a saved tokenizer or training. Import it lazily so training envs
...
class SelfiesTokenizer:
    ...
    def indexes_to_selfies(self, indexes):
        ...
        symbols = [ s for s in symbols if s not in self.special_tokens ]
        return ''.join(symbols)
